...
const CAPTION_PRODUCTS: ProductInfo[] = PRODUCT_CATALOG.map(p => ({
    name: p.name,
    brand: p.brand,
    benefit: p.benefitClaims[0] || 'kulit sehat',
    activeIngredient: p.ingredients[0] || 'Active Complex',
    price: p.marketPrice
}));
...
export const CopywritingForge = () => {
    const [selectedProduct, setSelectedProduct] = useState<ProductInfo>(CAPTION_PRODUCTS[0]);
        ...
        <div className="bg-gradient-to-br from-purple-950 to-slate-900 rounded-2xl p-6 text-white">
            ...
            <div className="mb-4">
                ...
                <select
                    ...
                    onChange={(e) => setSelectedProduct(CAPTION_PRODUCTS.find(p => p.name === e.target.value) || CAPTION_PRODUCTS[0])}
                ...
                <div className="grid grid-cols-2 gap-2">
                    {VIBES.map(v => (
                        <button
                            ...
                            className={`px-3 py-2 rounded-xl text-xs font-bold transition-all ${vibe === v.value ? 'bg-purple-500 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'
                                }`}
                        >
                            {v.emoji} {v.label}
                        </button>
            ...
            <div className="mb-4">
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all ${platform === 'INSTAGRAM' ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white' : 'bg-slate-800 text-slate-400'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all ${platform === 'TIKTOK' ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white' : 'bg-slate-800 text-slate-400'
                            }`}
                    >
                        🎵 TikTok
                    </button>
            ...
            {result && (
                <div className="bg-slate-800 rounded-xl p-4">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex gap-2">
                            <button onClick={handleGenerate} className="p-1.5 bg-slate-700 rounded-lg hover:bg-slate-600 transition-colors">
                                <RefreshCw size={14} />
                            </button>
                            <button
                                onClick={handleCopy}
